...
if st.session_state.get("initialized") and not st.session_state.get("round_over", False):
    ...
    st.markdown(f"""
        <div style="background: rgba(0,0,0,0.2); padding: 15px; border-radius: 15px; border: 1px solid #333; margin-bottom: 20px;">
            <div style="margin-bottom:10px;">
                <p style="margin:0; font-size:11px; color:#3a86ff; font-weight:bold;">BANK TIME ({bank_rem:.1f}s)</p>
                <div class="timer-container" style="height:8px; background:#111;"><div style="width:{bank_ratio*100}%; background:linear-gradient(90deg, #3a86ff, #00d4ff); height:100%;"></div></div>
            </div>
            <div>
                <p style="margin:0; font-size:12px; color:{t_color}; font-weight:bold;">TURN LIMIT ({actual_turn_rem:.1f}s)</p>
                <div class="timer-container" style="height:14px; background:#111; border: 1px solid #444;"><div style="width:{actual_turn_ratio*100}%; background:{t_color}; height:100%; box-shadow: 0 0 10px {t_color}66;"></div></div>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    for speaker, text in st.session_state.history:
        ...
        style = "border: 2px solid #FF0000; box-shadow: 0 0 10px rgba(255,0,0,0.4);" if "🔥" in text else ""
        chat_html += f'<div class="msg-row-{side}"><div class="{bub}" style="{style}">{text.replace("🔥","")}</div></div>'
    chat_html += '</div>'
    st.markdown(chat_html, unsafe_allow_html=True)

    # [F] 입력 폼 (이 안에서 else: 오류가 나지 않도록 구조 정렬)
    with st.form(key=f"game_input_{st.session_state.chain}", clear_on_submit=True):
        user_input = st.text_input("단어 입력", label_visibility="collapsed", placeholder="단어를 입력하고 Enter!")
...
# ────────────────────────────────────────────────
# 6. 라운드 종료 화면 (섹션 5의 if문이 끝난 뒤 위치)
# ────────────────────────────────────────────────
else:
    # 여기가 Line 251 근처입니다. 
    # 위쪽의 if st.session_state.get("initialized") ... 블록과 짝이 맞아야 합니다.
